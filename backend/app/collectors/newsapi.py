...
TARGET_COUNTRIES = [
    ...
    "de", "fr", "es", "it", "pt", "nl", "be", "at", "ch",  # Western Europe
    "se", "no", "dk", "fi", "is",  # Nordic
    "pl", "cz", "hu", "ro", "bg", "gr", "ua", "ru",  # Eastern Europe
    "jp", "kr", "cn", "tw", "hk", "sg", "my", "id", "th", "vn", "ph",  # Asia
    "in", "pk", "bd", "lk", "np",  # South Asia
    "il", "sa", "ae", "eg", "tr", "ir", "iq", "lb", "jo",  # Middle East
    "za", "ng", "ke", "et", "gh", "tz",  # Africa
...
class NewsDataCollector(BaseCollector):
    ...
    def _parse_article(self, item: dict, country: str) -> Optional[CollectedArticle]:
        ...
        try:
            ...
            if pubdate:
                try:
                    published_at = datetime.fromisoformat(pubdate.replace("Z", "+00:00"))
...
class CurrentsAPICollector(BaseCollector):
    ...
    async def collect(self) -> List[CollectedArticle]:
        ...
        # Fetch by region
        regions = ["us", "eu", "asia", "africa", "middle_east", "latin_america", "oceania"]
        for region in regions:
            try:
                region_articles = await self._fetch_region(region)
                articles.extend(region_articles)
            except Exception as e:
                logger.debug("Failed to fetch Currents region", region=region, error=str(e))
    ...
    async def _fetch_latest(self) -> List[CollectedArticle]:
        ...
        try:
            ...
            response = await self.client.get(f"{self.base_url}/latest-news", params=params)
            response.raise_for_status()
...
class TheNewsAPICollector(BaseCollector):
    ...
    async def collect(self) -> List[CollectedArticle]:
        ...
        for locale in locales:
            try:
                ...
                logger.debug("Failed to fetch TheNewsAPI locale", locale=locale, error=str(e))
    ...
    def _parse_article(self, item: dict, default_country: str = None) -> Optional[CollectedArticle]:
