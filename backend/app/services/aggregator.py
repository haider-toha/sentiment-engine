...
class SentimentAggregator:
    ...
    def aggregate_hourly(self, hour: Optional[datetime] = None) -> Dict[str, dict]:
        ...
        for country_code, data in country_data.items():
            ...
            weighted_sentiment = sum(s * w for s, w in data["weighted_scores"]) / total_weight
            ...
            existing = self.db.query(CountrySentiment).filter(
                and_(
        ...
        self.db.commit()
        logger.info("Aggregated hourly sentiment", countries=len(results), hour=hour_start)
    ...
    def get_global_sentiment(self) -> Dict[str, any]:
        ...
        latest = self.db.query(CountrySentiment).order_by(
            CountrySentiment.hour.desc()
        ).first()
        ...
        hour = latest.hour
        country_sentiments = self.db.query(CountrySentiment).filter(
            CountrySentiment.hour == hour
        ).all()
        ...
        for cs in country_sentiments:
            ...
            weighted_sum += (cs.weighted_sentiment or cs.avg_sentiment) * cs.article_count
    ...
    def get_country_detail(self, country_code: str, hours: int = 24) -> Optional[dict]:
        ...
        hourly = self.db.query(CountrySentiment).filter(
            and_(
                CountrySentiment.country_code == country_code,
                CountrySentiment.hour >= since,
            )
        ).order_by(CountrySentiment.hour.asc()).all()
    ...
    def cleanup_old_data(self, days: int = 30) -> int:
        ...
        # Delete old articles
        deleted_articles = self.db.query(Article).filter(
            Article.created_at < cutoff
        ).delete()
        
        # Delete old aggregates
        deleted_aggregates = self.db.query(CountrySentiment).filter(
            CountrySentiment.hour < cutoff
        ).delete()
