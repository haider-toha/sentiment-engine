...
export default function SentimentCard({
  ...
  const TrendIcon =
    trend && trend > 0.05 ? TrendingUp : trend && trend < -0.05 ? TrendingDown : Minus
      ...
      <CardContent className={cn('p-4', large && 'p-5')}>
        <div className="mb-3 flex items-start justify-between">
          ...
          {trend !== undefined && (
            <div
              className={cn(
                'flex items-center gap-0.5 rounded px-1.5 py-0.5 font-mono text-2xs font-medium',
                trend > 0.05
        ...
        {subtitle && <p className="mt-2 text-2xs text-text-muted">{subtitle}</p>}
      </CardContent>
    </Card>
  )
}
